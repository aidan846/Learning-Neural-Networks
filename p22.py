...
'''
P21 proved: "Can the network learn a sine wave from training points?"

P22 should answer: "Can it predict the sine wave at points it never trained on?"
'''
...
# Much fewer training points (larger step size)
X = np.arange(0, 1, 0.05).reshape(-1, 1)
y = np.sin(2 * np.pi * X)

# High-resolution validation set remains dense
X_val = np.arange(0, 1, 0.001).reshape(-1, 1)
...
actual_line, = ax.plot(X, y, label="Training Target")
prediction_line, = ax.plot(X, np.zeros_like(y), label="Training Prediction")
...
training_loss = loss_function.calculate(
    activation3.output,
    y
)
...
# Save predictions before validation overwrites activation3.output
...
validation_loss = loss_function.calculate(
    activation3.output,
    y_val
)
...
validation_predictions = activation3.output.copy()
...
# Training samples the model actually saw
ax_final.scatter(
    X,
    y,
    s=15,
...
# True function over dense validation set
ax_final.plot(
    X_val,
    y_val,
...
# Model predictions on unseen validation points
ax_final.plot(
    X_val,
